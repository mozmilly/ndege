...
public class CoreCategoryAdapter extends  RecyclerView.Adapter<CoreCategoryAdapter.MyViewHolder>{
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        if (coreCategoryList.get(position).getImage()!=null){
            if (URLUtil.isValidUrl("https://storage.googleapis.com/ndege/"+coreCategoryList.get(position).getImage())){
                Picasso.with(context)
                        .load("https://storage.googleapis.com/ndege/"+coreCategoryList.get(position).getImage())
