...
public class MenuCategoryAdapter extends RecyclerView.Adapter<MenuCategoryAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        if (menuCategoryList.get(position).getImage()!=null){
            if (URLUtil.isValidUrl("https://storage.googleapis.com/ndege/"+menuCategoryList.get(position).getImage())){
                Picasso.with(context)
                        .load("https://storage.googleapis.com/ndege/"+menuCategoryList.get(position).getImage())
